...
public class SmelteryFluidWrapper
{
    ...
    public SmelteryFluidWrapper(String name, int amount)
    {
        ...
        this.fluid = fluid;
        this.amount = amount;
        ItemStack blockStack = UniOreDictionary.getFirstEntry("block" + WordUtils.capitalize(this.name));
        if (blockStack != null) {
            this.block = (blockStack.getItem() instanceof ItemBlock) ? ((ItemBlock) blockStack.getItem()).block : null;
            this.blockMeta = blockStack.getItemDamage();
            this.pointOfFusion = 250;//(int) Math.round(Smeltery.getLiquifyTemperature(this.block, this.blockMeta) * 0.60);
        } else {
            this.block = null;
            this.blockMeta = 0;
